...
function DirectedGraph:topological_sort(opts)
	local sorting = {}

	-- copy self so edges can be removed without affecting the real graph.

	local graph, original_vert
	if opts.consume then
		graph, original_vert = graph_copy(self)
	else
		graph, original_vert = self, tbl_util.id_map(self.vertices)
	end
...
function LabeledDigraph:topological_sort(opts)
	return vim.tbl_map(function(v)
		return self.vert_to_label[v]
	end, self.graph:topological_sort(opts))
end

function LabeledDigraph:describe()
	local description = ""
	for _, vert in ipairs(self.graph.vertices) do
		for other_vert, _ in pairs(vert.outgoing_edge_verts) do
			description = description .. tostring(self.vert_to_label[vert]) .. " -> " .. tostring(self.vert_to_label[other_vert]) .. "\n"
		end
	end
	-- omit trailing \n
